...
const Calendario: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    useIonViewDidEnter(() => {
    // Forzar el redibujado de FullCalendar al volver al tab
    ...
    const calendarEvents = eventos.map(evento => ({
      ...
      ubicacion: evento.ubicacion,
      backgroundColor: evento.tipo === 'entrenamiento' ? '#ffc409' : 
            ...
            {eventosProximos.map((evento) => (
              <IonItem key={evento.id}>
                <IonLabel>
                  ...
                  <p>
                    <IonIcon icon={time} /> {evento.fecha_inicio.replace('T', ' ').slice(0, 16)}
                  </p>
                  <p>
