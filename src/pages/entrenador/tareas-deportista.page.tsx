...
const TareasDeportista: React.FC = () => {
  ...
  const handleCrearTarea = async () => {
    ...
    try {
      ...
      if (selectedDeportistas.length > 1) {
        await tareaService.crear_tarea_individual_multiple(
          formData.titulo.trim(),
          formData.descripcion.trim(),
          selectedDeportistas.map(d => d.id.toString()),
          user?.id || '',
        ...
        await tareaService.crear_tarea_individual(
          formData.titulo.trim(),
          formData.descripcion.trim(),
          formData.deportista_id,
          user?.id || '',
          formData.fecha_vencimiento,
          formData.estado,
          formData.req_eva
      ...
      setToastMessage('Error al crear la(s) tarea(s)');
    } finally {
      setLoading(false);
      setShowToast(true);
    }
  ...
  return (
    <IonPage>
      <AppHeader title="Tareas de Deportista" />
          ...
          {tareasFiltradas.map((tarea) => (
            <IonItem key={tarea.id} button onClick={() => handleTareaClick(tarea.id)}>
              ...
              <IonLabel>
                ...
                <p>Deportista: {getDeportistaNombre(tarea.deportista_id)}</p>
              </IonLabel>
              <IonButtons slot="end">
                <IonButton onClick={(e) => {
                  e.stopPropagation();
